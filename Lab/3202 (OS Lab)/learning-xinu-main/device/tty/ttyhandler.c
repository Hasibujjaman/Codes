...
void ttyhandler(void) {
	...
	/* to coordinate with the upper half of the driver		*/
		...
		while ( (io_inb(csrptr->lsr) & UART_LSR_DR) != 0) {
			ttyhandle_in(typtr, csrptr);   // the buffer register(csptr->buffer) serves as the access point to read data from the UART's input FIFO. The hardware automatically moves the next byte from its input FIFO into this buffer register when it's read.
                }
	    ...
	    case UART_IIR_THRE:
		ttyhandle_out(typtr, csrptr);  // writes data to the buffer register(csptr->buffer), which the UART hardware then transfers into its output FIFO for transmission.
		return;
