	...
	)
{
	...
	if ( (typtr->tyehead == typtr->tyetail) &&
	     (semcount(typtr->tyosem) >= TY_OBUFLEN) ) { // semaphore count of the output buffer semaphore indicates the number of free slots in the output buffer. 
	...
	while ( (uspace>0) &&  typtr->tyehead != typtr->tyetail) {
		io_outb(csrptr->buffer, *typtr->tyehead++);  // writes data(a byte) to the UART's buffer(csptr->buffer) which is inside the CSR(csrptr) of the UART, which the UART hardware then transfers into its output FIFO for transmission.
